...
* :math:`q_n`; 
* :math:`f`;
* `определяющие соотношения среды`: :math:`\vec{j} = - \hat{\lambda} \nabla T`,
...
В задаче теплопроводности определяющие соотношения связывают вектор плотности теплового потока :math:`\vec{j}` и градиент
температуры :math:`\nabla T`:
